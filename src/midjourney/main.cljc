(ns midjourney.main
  "Kotodama WASM entrypoint for the Midjourney clean-room actor (L4) — Clojure port.
  ...
  facts (`midjourney.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "midjourney")
(def tier "L4")
...
(def entity-specs
  [{:entity "Model"      :plural "models"      :id-prefix "midjourn_mod"
    :fields [:name :family :contextWindow :modality]        :required [:name :family]
    :coerce {:contextWindow :int}             :refs {}}
   {:entity "Completion" :plural "completions" :id-prefix "midjourn_com"
    :fields [:modelId :prompt :output :tokens]              :required [:prompt :output]
    :coerce {:tokens :int}                    :refs {:modelId "Model"}}
   {:entity "Embedding"  :plural "embeddings"  :id-prefix "midjourn_emb"
    :fields [:modelId :input :dimensions :vectorRef]        :required [:input :dimensions]
    :coerce {:dimensions :int}                :refs {:modelId "Model"}}
   {:entity "FineTune"   :plural "finetunes"   :id-prefix "midjourn_fin"
    :fields [:baseModelId :datasetId :status :epochs]       :required [:status :epochs]
    :coerce {:epochs :int}                    :refs {:datasetId "Dataset"}}
   {:entity "Dataset"    :plural "datasets"    :id-prefix "midjourn_dat"
    :fields [:name :rows :contentRef]                       :required [:name :rows]
    :coerce {:rows :int}                      :refs {}}
   {:entity "File"       :plural "files"       :id-prefix "midjourn_fil"
    :fields [:purpose :filename :sizeBytes :contentRef]     :required [:purpose :filename]
    :coerce {:sizeBytes :int}                 :refs {}}])
...
(defn emit-facts
  "EAVT facts for one record: {\"midjourney.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "midjourney-compat" :tier tier :entities entities} 200])
...
(defn start! [] :midjourney-compat/ready)
